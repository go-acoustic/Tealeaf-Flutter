...
class PluginTealeaf {
  ...
  static Future<String> get platformVersion async {
    ...
    on PlatformException catch (pe) {
      throw TealeafException(pe, msg: 'Unable to process platform version request message!');
    }
  }
  ...
  static Future<String> get tealeafVersion async {
    ...
    on PlatformException catch (pe) {
      throw TealeafException(pe, msg: 'Unable to process TeaLeaf request version message!');
    }
  }

  static Future<String> get tealeafSessionId async {
    try {
      final String sessionId = await _channel.invokeMethod('getTealeafSessionId');
      tlLogger.v("Tealeaf sessionId: $sessionId");
      return sessionId;
    }
    on PlatformException catch (pe) {
      throw TealeafException(pe, msg: 'Unable to process Tealeaf request sessionId message!');
    }
  }
  ...
  static Future<String> get appKey async {
    ...
    on PlatformException catch (pe) {
      throw TealeafException(pe, msg: 'Unable to process app key request message!');
    }
  }
  ...
  static Future<void> tlSetEnvironment({@required int screenWidth, @required int screenHeight}) async {
    ...
    on PlatformException catch (pe) {
      throw TealeafException(pe, msg: 'Unable to send Flutter screen parameters message!');
    }
  }
  ...
  static Future<void> tlApplicationCustomEvent({@required String eventName, Map<String, String> customData, int logLevel}) async {
    ...
    on PlatformException catch (pe) {
      throw TealeafException(pe, msg: 'Unable to process custom event message!');
    }
  }
  ...
  static Future<void> tlApplicationCaughtException({Exception caughtException, StackTrace stack, Map<String,String> appData}) async {
    ...
    on PlatformException catch (pe) {
      throw TealeafException(pe, msg: 'Unable to process user caught exception message!');
    }
  }
  ...
  static Future<void> onTlException({@required Map<dynamic, dynamic> data}) async {
    ...
    on PlatformException catch (pe) {
      tlLogger.v('Unable to log app exception: ${pe.message}, stack: ${pe.stacktrace}');
  ...
  static Future<void> onTlPointerEvent({@required Map fields}) async {
    ...
    on PlatformException catch (pe, stack) {
      tlLogger.v("pointerEvent exception: ${pe.toString()}, stack: ${stack.toString()}");
      throw TealeafException(pe, msg: 'Unable to process flutter pointer event message!');
    }
  }
    ...
    try {
      if (["pinch", "swipe", "taphold", "doubletap", "tap"].contains(gesture)) {
        return await _channel.invokeMethod(
          'gesture',
          <dynamic, dynamic>{'tlType': gesture, 'id': id, 'target': target, 'data': data, 'layoutParameters': layoutParameters}
        );
      }
      throw TealeafException.create(code: 3, msg: 'Illegal gesture type: "$gesture"');
    }
    on PlatformException catch (pe) {
    ...
    try {
      if (["LOAD", "UNLOAD", "VISIT"].contains(tlType)) {
        ...
        return await _channel.invokeMethod('screenview', <dynamic, dynamic>
          {'tlType': tlType, 'timeStamp': timeString, 'layoutParameters': layoutParameters});
      }

      throw TealeafException.create(code: 2, msg: 'Illegal screenview transition type');
    }
    on PlatformException catch (pe) {
      throw TealeafException(pe, msg: 'Unable to process screen view (update) message!');
    }
  }
  ...
  static Future<String> getGlobalConfiguration() async {
    ...
    on PlatformException catch (pe) {
      throw TealeafException(pe, msg: 'Unable to process global configuration settings message!');
    }
  }
  ...
  static Future<String> maskText(String text, [String page]) async {
    ...
    on PlatformException catch (pe) {
      throw TealeafException(pe, msg: 'Unable to process string masking request!');
    }
  }
