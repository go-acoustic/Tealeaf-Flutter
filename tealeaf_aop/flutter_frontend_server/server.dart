...
class _FlutterFrontendCompiler implements frontend.CompilerInterface {
  ...
  Future<bool> compile(String filename, ArgResults options,
    ...
    /*  4/20/22
     *  Fixed null pointer crash when incremental compilation specified.
     */
    final bool incremental = options.arguments.contains('--incremental');

    MyLog.p('*** Compile: incremental=$incremental');
    ...
    if (!transformers.contains(aspectdAopTransformer)) {
      MyLog.p("Adding aspectd transformers");
      transformers.add(aspectdAopTransformer);
    } else {
      MyLog.p("NOT Adding aspectd transformers");
    }
    //}

    /*
    final List<FlutterProgramTransformer> transformers = FlutterTarget.flutterProgramTransformers;
    if (!transformers.contains(aspectdAopTransformer)) {
      transformers.add(aspectdAopTransformer);
    }
    */
      ...
      node.function.body.replaceWith(
        ReturnStatement(
          SuperMethodInvocation(
              ...
              // SJS: 3rd parameter required with Flutter 3.3.0/Dart 2.18.0
              //      The type is Procedure (which extends Member) so node is logical
              //      candidate. Actually since it was null before, it probably is not used
              //      for our purposes.
              node),
        ),
      );
